...
class AutoScroller:
    ...
    def create_widgets(self):
        ...
        container.pack(fill='both', expand=True, padx=48, pady=(72, 48))

        middle_frame = tk.Frame(container)
        middle_frame.pack(fill='both', expand=True)

        # === CLICKER SECTION ===
        self.clicker_button = ttk.Button(middle_frame, text=self.locale.get('button_start_click'), command=self.toggle_clicker)
        ...
        self.start_stop_button = ttk.Button(middle_frame, text=self.locale.get('button_start_scroll'), command=self.toggle_scroll)
        ...
        self.smart_pause_status_label.pack(pady=(6, 0))
    ...
    def _do_hold_click(self):
        ...
        try:
            ...
            # Hold for duration (if 0, hold until manually stopped)
            if duration > 0:
                import time
                time.sleep(duration)
                ...
                # Schedule next hold click - l'intervalle commence MAINTENANT (après relâchement)
                # Ne pas appeler _schedule_next_click() directement car ça commence l'intervalle avant le hold
                # À la place, générer l'intervalle et programmer l'exécution
                try:
                    minv = float(self.click_min_var.get())
                except Exception:
                    minv = 1.0
                ...
                # Infinite hold - stop clicker but don't release
                self._log(self.locale.get('log_hold_infinite'))
                # Don't schedule next click, user must manually stop
    ...
    def _start_global_listeners(self):
        ...
        if _HAS_KEYBOARD and _pynput_keyboard is not None:
            try:
                self._start_keyboard_listener()
            except Exception:
                pass
    ...
    def _start_keyboard_listener(self):
        ...
        if not _HAS_KEYBOARD or _pynput_keyboard is None:
            return
